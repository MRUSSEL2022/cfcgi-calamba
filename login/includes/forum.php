...
$id = $_SESSION['user_id'];
$sql = "SELECT surname FROM user WHERE user_id = $id";
$result = mysqli_query($conn, $sql);
...
?> <div class="container"> <?php
    ...
    // Get all the posts from the database
    $sql = "SELECT * FROM posts WHERE userId = $id ORDER BY created_at DESC";
    $result = mysqli_query($conn, $sql); ?> <?php // Check if any posts are found
    if (mysqli_num_rows($result) > 0) {
        // Loop through each row and display the post title, content, and created date
    ...
    <div class="widget-post" aria-labelledby="post-header-title">
      ...
      <form action="submit_post.php" method="post" id="widget-form" class="widget-post__form" name="form" aria-label="post widget">
        <div class="widget-post__content">
          <label for="post-content" class="sr-only">asdasdsa</label> <?php echo $row['content']; ?>
        </div>
        <div class="widget-post__options is--hidden" id="stock-options"></div>
        <div class="widget-post__actions post--actions">
          <div class="post-actions__attachments">
            <button type="button" class="btn post-actions__upload attachments--btn"> share </button>
          </div>
          <div class="post-actions__widget"> <?php echo date('F j, Y, g:i a', strtotime($row['created_at'])); ?> </div>
        </div>
      </form>
    </div>
    <br>
